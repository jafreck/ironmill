...
/// Pre-compiled for common group sizes (32, 64, 128, 256).
pub struct GroupSizePipelines {
    /// Map from group size → specialized compute pipeline.
    pub variants: HashMap<u32, ComputePipeline>,
}

impl GroupSizePipelines {
    /// Look up the pipeline for a given group size, or `None` if unsupported.
