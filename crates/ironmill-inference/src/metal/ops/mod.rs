...
use ironmill_metal_sys::{ComputePipeline, FunctionConstantValues, MetalDevice, ShaderLibrary};
...
/// Group-size-specialized pipeline variants.
///
/// Pre-compiled for common group sizes (32, 64, 128, 256) so the Metal compiler
/// can optimize `/ GS` → `>> log2(GS)` and `% GS` → `& (GS-1)` at compile time.
pub struct GroupSizePipelines {
    variants: std::collections::HashMap<u32, ComputePipeline>,
}

impl GroupSizePipelines {
    /// Get the pipeline variant for a specific group size.
    pub fn get(&self, group_size: u32) -> Option<&ComputePipeline> {
        self.variants.get(&group_size)
    }
}

/// Create specialized pipeline variants for each supported group size.
fn make_gs_pipelines(
    ...
    for &gs in &[32u32, 64, 128, 256] {
        let constants = FunctionConstantValues::new().map_err(MetalError::Metal)?;
        constants.set_u32(gs, 0);
        let func = lib
            .get_function_with_constants(name, &constants)
            .map_err(MetalError::Metal)?;
        let pipeline = device
...
impl MetalPipelines {
    ...
    fn compile_advanced_matmul_shaders(
        ...
        Ok((
            ...
            // Superblock pipelines (with GS function constant)
            make_gs_pipelines(device, &libs.superblock, "superblock_matvec_int4")?,
            make_gs_pipelines(device, &libs.superblock, "superblock_matmul_int4")?,
            make_gs_pipelines(
                device,
                &libs.superblock,
            ...
            make_gs_pipelines(device, &libs.superblock, "superblock_affine_matvec_int4xq8")?,
            make_gs_pipelines(device, &libs.superblock, "superblock_matvec_int8")?,
            make_gs_pipelines(device, &libs.superblock, "superblock_matmul_int8")?,
        ))
    }
    ...
    /// Select the affine-quantized pipeline for a given bit-width, phase, and group size.
    #[inline]
