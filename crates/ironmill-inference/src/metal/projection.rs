            ...
            if let Some(q8_input) = q8 {
                if kernel_kind.is_decode() && aq.bit_width == 4 && aq.awq_scales.is_none() {
                    ops::encode_affine_matvec_int4xq8(
                        enc,
                        pipelines.affine.matvec_int4xq8.get(aq.group_size).unwrap(),
                        q8_input.data,
                        q8_input.scales,
                        aq,
                        output_buf,
    ...
    let pipeline = pipelines
        ...
        .ok_or_else(|| {
            InferenceError::runtime(format!(
                "unsupported affine bit_width: {} / group_size: {}",
                weight.bit_width, weight.group_size
            ))
        })?;

    encoder.set_pipeline(pipeline);
    encoder.set_buffer(input, 0, 0);
    encoder.set_buffer(&weight.data, 0, 1); // superblock
