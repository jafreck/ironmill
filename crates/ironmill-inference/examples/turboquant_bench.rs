...
fn main() {
    ...
    let tq_alloc = model.alloc_size();

    let mut q = AneTensor::new_with_min_alloc(q_ch, 1, ScalarType::Float16, tq_alloc).unwrap();
    let mut k_proj =
        AneTensor::new_with_min_alloc(kv_ch, 1, ScalarType::Float16, tq_alloc).unwrap();
    let mut v_proj =
        AneTensor::new_with_min_alloc(kv_ch, 1, ScalarType::Float16, tq_alloc).unwrap();
    ...
    let projections: Vec<(AneTensor, AneTensor, AneTensor)> = (0..NUM_LAYERS)
        .map(|_| {
            let mut q =
                AneTensor::new_with_min_alloc(q_ch, 1, ScalarType::Float16, tq_alloc).unwrap();
            let mut k =
                AneTensor::new_with_min_alloc(kv_ch, 1, ScalarType::Float16, tq_alloc).unwrap();
            let mut v =
                AneTensor::new_with_min_alloc(kv_ch, 1, ScalarType::Float16, tq_alloc).unwrap();
    ...
    // --- Summary ---
    // INT8 cache: 1 byte/elem, 2× smaller than FP16 baseline.
    let cache_bytes = 2 * NUM_LAYERS * kv_ch * MAX_SEQ_LEN; // INT8: 1 byte/elem, K+V
    let cache_fp16_bytes = cache_bytes * 2; // FP16: 2 bytes/elem
    println!("  ═══════════════════════════════════════════════════════════");
    println!("  Summary (Qwen3-0.6B, seq_len={MAX_SEQ_LEN}):");
    println!(
        "    KV cache (TQ):    {:.1} MB (INT8, {:.0}% of FP16)",
        cache_bytes as f64 / 1024.0 / 1024.0,
        cache_bytes as f64 / cache_fp16_bytes as f64 * 100.0
    );
    println!(
