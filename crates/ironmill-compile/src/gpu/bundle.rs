...
use crate::weights::{ModelConfig, QuantizationInfo, WeightProvider};
...
pub fn str_to_scalar_type(s: &str) -> Result<ScalarType, CompileError> {
    match s {
        ...
        _ => Err(CompileError::Other(format!("unknown scalar type: {s}"))),
    }
}
...
pub fn deserialize_model_config(value: &serde_json::Value) -> Result<ModelConfig, CompileError> {
    use crate::weights::Architecture;

    let obj = value
        .as_object()
        .ok_or_else(|| CompileError::Other("model_config must be an object".into()))?;

    let get_str = |key: &str| -> Result<&str, CompileError> {
        obj.get(key)
            .and_then(|v| v.as_str())
            .ok_or_else(|| CompileError::Other(format!("missing string field: {key}")))
    };

    let get_usize = |key: &str| -> Result<usize, CompileError> {
        obj.get(key)
            .and_then(|v| v.as_u64())
            .map(|v| v as usize)
            .ok_or_else(|| CompileError::Other(format!("missing integer field: {key}")))
    };

    let get_f64 = |key: &str| -> Result<f64, CompileError> {
        obj.get(key)
            .and_then(|v| v.as_f64())
            .ok_or_else(|| CompileError::Other(format!("missing float field: {key}")))
    };

    let architecture: Architecture = get_str("architecture")?
        .parse()
        .map_err(|e: mil_rs::MilError| CompileError::Other(e.to_string()))?;
