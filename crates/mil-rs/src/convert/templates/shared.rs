...
use crate::ir::{Block, Operation, ScalarType, Value};
...
// ---------------------------------------------------------------------------
// Transformer layer (standard LLaMA-style)
// ---------------------------------------------------------------------------

/// Emit a standard transformer layer: RMSNorm → Attention → Residual →
/// RMSNorm → MLP(SiLU) → Residual.
#[allow(clippy::too_many_arguments)]
#[allow(dead_code)]
pub(super) fn emit_transformer_layer(
    block: &mut Block,
    provider: &dyn WeightProvider,
    config: &ModelConfig,
    layer_idx: usize,
...
impl WeightProvider for StubProvider {
    fn tensor(&self, name: &str) -> Result<crate::convert::weights::WeightTensor<'_>, MilError> {
        let data = self
            .tensors
            .get(name)
        ...
        Ok(crate::convert::weights::WeightTensor::borrowed(
            data,
            vec![num_elements],
            ScalarType::Float16,
        ))
