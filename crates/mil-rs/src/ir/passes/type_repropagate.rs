...
/// Infer conv output: `[N, out_channels, out_h, out_w]`.
fn infer_conv_output(op: &Operation, type_map: &HashMap<String, TensorType>) -> Option<TensorType> {
    ...
    let strides = read_int_list_input(op, "strides").unwrap_or_else(|| vec![1, 1]);
    let dilations = read_int_list_input(op, "dilations").unwrap_or_else(|| vec![1, 1]);
    let pads = read_int_list_input(op, "pad").unwrap_or_else(|| vec![0, 0, 0, 0]);
...
/// Infer pool output: channels preserved, spatial dims shrink.
fn infer_pool_output(op: &Operation, type_map: &HashMap<String, TensorType>) -> Option<TensorType> {
    ...
    let kernels = if is_global {
        ...
        read_int_list_input(op, "kernel_sizes").unwrap_or_else(|| vec![1, 1])
    };
    let strides = read_int_list_input(op, "strides").unwrap_or_else(|| vec![1, 1]);
    let pads = read_int_list_input(op, "pad").unwrap_or_else(|| vec![0, 0, 0, 0]);
...
/// Infer concat output: all inputs must share the same shape except along the
/// concatenation axis.
fn infer_concat_output(
    ...
    let mut axis_sum: usize = 0;
    for tt in &types {
        axis_sum += tt.shape.get(axis).copied().flatten().unwrap_or(0);
    }
...
/// Read an int list from a `Value::Tensor` with Int32 data.
fn read_int_list(value: &Value) -> Option<Vec<i64>> {
    if let Value::Tensor {
        data,
...
/// Read an int list from an op input.
fn read_int_list_input(op: &Operation, name: &str) -> Option<Vec<i64>> {
    op.inputs.get(name).and_then(read_int_list)
}
