...
use crate::ir::operation::ComputeUnit;
...
///
/// The pass is non-destructive — it only sets the `compute_unit` field on
    ...
    fn run(&self, program: &mut Program) -> Result<()> {
        for function in program.functions.values_mut() {
            ...
            for op in &mut function.body.operations {
                ...
                if op.op_type == "const" {
                    op.compute_unit = Some(ComputeUnit::Any);
                    continue;
                }
                ...
                if !is_ane_supported(&op.op_type) {
                    ...
                    op.compute_unit = Some(ComputeUnit::Cpu);
                    continue;
                }
                ...
                if shape_rejection.is_some() {
                    ...
                    op.compute_unit = Some(ComputeUnit::Gpu);
                    continue;
                }
                ...
                // Fully ANE-eligible.
                op.compute_unit = Some(ComputeUnit::Ane);
            }
        }
        Ok(())
    }
