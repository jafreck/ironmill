...
use crate::compiler::{compile_model, is_compiler_available};
use crate::convert::{
    ConversionConfig, model_to_program, onnx_to_program_with_config, program_to_model,
};
use crate::error::{MilError, Result};
use crate::ir::{PassPipeline, PipelineReport};
use crate::reader::{read_mlmodel, read_mlpackage, read_onnx};
use crate::writer::write_mlpackage;
    ...
    pub fn build(self) -> Result<BuildOutput> {
        ...
        let mut program = match detect_format(input) {
            ...
            InputFormat::SafeTensors => {
                let provider = crate::reader::read_safetensors(input)?;
                let result = crate::convert::weights_to_program(&provider)?;
                result.program
            }
            InputFormat::Gguf => {
                let provider = crate::reader::read_gguf(input)?;
                let result = crate::convert::weights_to_program(&provider)?;
                result.program
            }
