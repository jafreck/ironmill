...
`MilWeightProvider`. Peak memory reaches roughly **3× model size**.
...
- Changes to the inference runtime (`ironmill-inference`).
...
All four existing implementors store only owned data (`HashMap`, `Vec`, `Mmap`
from `memmap2`) which are `Send + Sync`. No implementation changes required.
...
```rust
// Before extracting the tensor value, materialize if external:
let val_location = if in_inputs { "inputs" } else { "attributes" };
let val_map = if in_inputs { &mut op.inputs } else { &mut op.attributes };

if let Some(Value::Tensor { ref mut data, .. }) = val_map.get_mut("val") {
    data.materialize_with(|key| program.resolve_tensor(key))?;
}

// Then proceed with existing removal + quantization logic unchanged.
let val = val_map.remove("val").ok_or_else(|| ...)?;
```

Note: passes take `&mut Program`, so they have access to `program.resolve_tensor()`
for materialization. The borrow checker is satisfied because we materialize the
tensor data in place (mutating `op.inputs`/`op.attributes`), then proceed with
the existing removal logic.

However, the current pass signatures iterate `program.functions.values_mut()` and
need `program.resolve_tensor()` simultaneously. Since `resolve_tensor()` only
reads the `weight_provider` field (not `functions`), this is safe — but the borrow
checker may not see it. The solution: extract the provider `Arc` before iterating:
...
fn run(&self, program: &mut Program) -> Result<()> {
    ...
    for function in program.functions.values_mut() {
        for op in &mut function.body.operations {
            // ... materialize_with(|key| resolve(key)) ...
        }
    }
    Ok(())
}
    ...
    for op in &mut function.body.operations {
        ...
        if let Some(Value::Tensor { ref mut data, shape, dtype }) = val {
            let bytes = match std::mem::replace(data, TensorData::inline(Vec::new())) {
                ...
                TensorData::External { provider_key, .. } => {
                    program.resolve_tensor(&provider_key)?
                }
            };
...
## 9. `GpuCompileBuilder` Integration
