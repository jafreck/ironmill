...
Gemma 4 via GGUF is explicitly rejected. The GGUF metadata format lacks the
per-layer attention metadata (`layer_types`, `rope_parameters`,
`global_head_dim`) required for correct Gemma 4 compilation. Without these,
global layers would receive wrong RoPE frequencies and head dimensions,
producing silently incorrect output.

An explicit guard in `extract_model_config` (`crates/ironmill-compile/src/weights/gguf.rs`)
returns a clear error directing users to SafeTensors format. This guard covers
both `gemma4` and `gemma4_text` architecture strings.

Use SafeTensors format for all Gemma 4 models.
...
- `cargo test -p ironmill-compile --lib` — existing tests must not regress
